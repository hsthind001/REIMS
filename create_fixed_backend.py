# Read the original file
with open('simple_backend.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Find the position before the first if __name__ block
lines = content.split('\n')
main_block_index = None

for i, line in enumerate(lines):
    if line.strip() == 'if __name__ == "__main__":':
        main_block_index = i
        break

if main_block_index is None:
    print("Could not find if __name__ block")
    exit(1)

# Extract the routes section (from the comment to the end of the second function)
routes_start = None
routes_end = None

for i, line in enumerate(lines):
    if 'Year-based Financial Data Endpoints' in line:
        routes_start = i
    elif routes_start is not None and 'def extract_year_from_filename' in line:
        # Find the end of this function
        for j in range(i, len(lines)):
            if lines[j].strip() == '' and j > i + 5:  # Empty line after function
                routes_end = j
                break
        break

if routes_start is None or routes_end is None:
    print("Could not find routes section")
    exit(1)

# Extract routes
routes_lines = lines[routes_start:routes_end]

# Remove routes from original position
lines_without_routes = lines[:routes_start] + lines[routes_end:]

# Insert routes before the main block
final_lines = lines_without_routes[:main_block_index] + routes_lines + lines_without_routes[main_block_index:]

# Write the fixed file
with open('simple_backend_fixed.py', 'w', encoding='utf-8') as f:
    f.write('\n'.join(final_lines))

print("Fixed file created: simple_backend_fixed.py")
print(f"Original lines: {len(lines)}")
print(f"Fixed lines: {len(final_lines)}")
